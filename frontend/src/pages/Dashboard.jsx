...
export default function Dashboard({ user, onLogout }) {
  ...
  const handleDeposit = async () => {
    try {
      await deposit(user.accountNumber, Number(amount));
      await fetchBalance();
      setAmount("");
    } catch (error) {
      console.error(error);
  ...
  const handleWithdraw = async () => {
    try {
      await withdraw(user.accountNumber, Number(amount));
      await fetchBalance();
      setAmount("");
    } catch {
    ...
    <div className="min-h-screen bg-gradient-to-r from-indigo-600 to-purple-700 flex items-center justify-center">
      <div className="bg-white w-[450px] p-8 rounded-2xl shadow-2xl">
        ...
        <div className="bg-indigo-50 rounded-xl p-5 mb-6 text-center">
          ...
          <h2 className="text-4xl font-bold text-indigo-600">₹{balance}</h2>
        </div>
